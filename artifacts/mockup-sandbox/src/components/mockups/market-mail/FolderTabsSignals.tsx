...
type Market = { id: string; label: string; contact: string; state: string; unread?: boolean; review?: boolean; sender: string; initials: string; message: string };
...
const markets: Market[] = [
  ...
  { id: "cornerstone", label: "Cornerstone (Demo)", contact: "Nia Patel · Underwriting", state: "Quote received", unread: true, review: true, sender: "Nia Patel", initials: "NP", message: "We can support the primary class mix. The indication is ready for staff review after the loss-run clarification." },
  { id: "decision", label: "Decision HR", contact: "Rafael Ortiz · Market contact", state: "Matched", review: true, sender: "Rafael Ortiz", initials: "RO", message: "Decision HR has matched the appetite profile. We are reviewing the requested effective date." },
...
export function FolderTabsSignals() {
  ...
  const reviewNeeded = market.review && !reviewed.includes(market.id);
  ...
  const markReviewed = () => setReviewed((items) => items.includes(market.id) ? items : [...items, market.id]);

  return <main className={`folder-tabs-prototype${light ? " light" : ""}`}>
    <style>{`.folder-tabs-prototype .review-signal{width:7px;height:7px;border:1px solid var(--pink);border-radius:50%;background:transparent;box-shadow:0 0 0 2px var(--pink-soft)}.folder-tabs-prototype .review-note{margin:8px 0 0 32px;padding:7px 8px;border-radius:6px;border:1px solid rgba(233,30,140,.38);background:var(--pink-soft);color:var(--ink);font-size:10px;display:flex;align-items:center;justify-content:space-between;gap:8px}.folder-tabs-prototype .review-note button{border:0;border-radius:4px;padding:4px 7px;background:var(--pink);color:#fff;font-size:9px;font-weight:700;cursor:pointer;white-space:nowrap}.folder-tabs-prototype.light .review-note{color:#3f1230}`}</style>
    <div className="mtp-backdrop"><section className="mtp-dialog" aria-label="Deal dialog prototype">
      <header className="mtp-header">
        <div className="mtp-topline"><span className="prototype-chip">Prototype · folder tab signals</span><button className="theme-switch" onClick={() => setLight((value) => !value)} aria-label={`Switch to ${light ? "dark" : "light"} theme`}>{light ? <Moon size={15} /> : <Sun size={15} />}</button></div>
      ...
      <div className="mtp-body">
        ...
        <section className="mtp-main">
          <div className="market-tabs" role="tablist" aria-label="Market correspondence folders">{markets.map((item, index) => {
            const itemReviewNeeded = item.review && !reviewed.includes(item.id);
            return <button ref={(node) => { refs.current[index] = node; }} key={item.id} role="tab" id={`market-tab-${item.id}`} aria-selected={active === item.id} aria-controls={`market-panel-${item.id}`} tabIndex={active === item.id ? 0 : -1} className="market-tab" onClick={() => setActive(item.id)} onKeyDown={(event) => onTabKey(event, index)}>
              {item.label}{item.unread && <i className="unread" aria-label="Unread" />}{itemReviewNeeded && <i className="review-signal" aria-label="Review needed" />}
            </button>;
          })}</div>
          <div className="market-panel" role="tabpanel" id={`market-panel-${market.id}`} aria-labelledby={`market-tab-${market.id}`}>
            ...
            <div className="feed-head"><h2>{market.id === "general" ? "Deal correspondence" : "Market correspondence"}</h2><span>{reviewNeeded ? "Review requested" : "Overview"}</span></div>
            ...
            {reviewNeeded && <div className="review-note"><span>Staff review needed before placement decision.</span><button onClick={markReviewed}>Mark reviewed</button></div>}
...
export default FolderTabsSignals;