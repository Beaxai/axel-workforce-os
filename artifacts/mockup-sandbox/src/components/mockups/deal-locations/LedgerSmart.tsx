...
import { MapPin, Plus, Trash2, ChevronDown, ChevronRight, AlertTriangle, Search, CheckCircle2, Edit2, Sparkles } from 'lucide-react';
...
const SEED_DATA: Location[] = [
  { id: "1", street1: "4820 Harvest Rd", city: "Salinas", state: "CA", zip: "93901", classCodesCount: 3, employeesCount: 24, payroll: "$1.2M", mode: 'smart' },
  { id: "2", street1: "112 Commerce Way", street2: "Suite B", city: "Portland", state: "OR", zip: "97202", classCodesCount: 2, employeesCount: 15, payroll: "$850k", mode: 'smart' },
  { id: "3", street1: "77 Meridian Ave", city: "Denver", state: "CO", zip: "80014", classCodesCount: 4, employeesCount: 42, payroll: "$2.1M", mode: 'manual' }
];

export function LedgerSmart() {
  const [locations, setLocations] = useState<Location[]>(SEED_DATA);
  const [expanded, setExpanded] = useState<Set<string>>(new Set(["2", "3"]));
  const [smartInput, setSmartInput] = useState("112 Commer");
  ...
  const update = (id: string, field: keyof Location, val: string | EntryMode) => {
  ...
  return (
    <div className="flex flex-col gap-6 w-full max-w-5xl mx-auto p-8 bg-[#0f0f13] min-h-screen text-white font-sans antialiased">
      ...
      <div className="flex flex-col border border-white/[0.08] rounded-xl bg-white/[0.02] shadow-2xl overflow-hidden backdrop-blur-xl">
        ...
        <div className="flex flex-col">
          {locations.map((loc, i) => {
            ...
            const isActivelyTyping = isExpanded && loc.mode === 'smart' && loc.id === "2"; // Hardcode ID 2 for the active demo state
                      ...
                      {loc.mode === 'smart' ? (
                        ...
                        <div className="flex flex-col gap-4">
                          <div className="relative">
                            ...
                            <input
                              type="text"
                              value={isActivelyTyping ? smartInput : addressString}
                              onChange={(e) => isActivelyTyping && setSmartInput(e.target.value)}
                              className={`w-full bg-[#0f0f13] border ${isActivelyTyping ? 'border-[#E91E8C]/50 ring-1 ring-[#E91E8C]/20 shadow-[0_0_15px_rgba(233,30,140,0.05)]' : 'border-white/10'} rounded-lg pl-9 ${!isActivelyTyping ? 'pr-9' : 'pr-4'} py-2 text-[13px] text-white focus:outline-none focus:border-[#E91E8C] focus:ring-1 focus:ring-[#E91E8C] transition-all placeholder:text-white/20 shadow-inner`}
                              placeholder="Start typing an address..."
                              autoFocus={isActivelyTyping}
                            />
                            {!isActivelyTyping && addressString && (
                              <div className="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
                                <CheckCircle2 className="h-4 w-4 text-[#E91E8C]/80" />
                              </div>
                            )}

                            {/* Suggestion Dropdown (Demo for loc 2) */}
                            {isActivelyTyping && (
                              <div className="absolute top-[calc(100%+6px)] left-0 right-0 bg-[#15151a] border border-white/10 rounded-lg overflow-hidden shadow-[0_10px_30px_rgba(0,0,0,0.5)] z-30">
                                <div className="p-1">
                                  <button className="w-full text-left px-3 py-2.5 text-[13px] rounded-md flex items-center gap-3 transition-colors group bg-white/5">
                                    <MapPin className="h-4 w-4 text-[#E91E8C]" />
                                    <div>
                                      <div className="text-white font-medium">112 Commerce Way<span className="text-white/50 font-normal"> Suite B</span></div>
                                      <div className="text-white/40 text-[11px] mt-0.5">Portland, OR 97202</div>
                                    </div>
                                  </button>
                                  <button className="w-full text-left px-3 py-2.5 text-[13px] hover:bg-white/5 rounded-md flex items-center gap-3 transition-colors group">
                                    <MapPin className="h-4 w-4 text-white/30 group-hover:text-white/50" />
                                    <div>
                                      <div className="text-white/70 group-hover:text-white">112 Commercial St</div>
                                      <div className="text-white/40 text-[11px] mt-0.5">Salem, OR 97301</div>
                                    </div>
                                  </button>
                                </div>
                                <div className="bg-black/20 border-t border-white/5 px-3 py-2 text-[11px] text-white/40 flex justify-between items-center">
                                  <span>Powered by Mapbox</span>
                                  <button onClick={() => update(loc.id, 'mode', 'manual')} className="text-[#E91E8C] hover:text-[#E91E8C]/80 font-medium transition-colors">Enter manually</button>
                                </div>
                              </div>
                            )}
                          </div>

                          {/* Parsed Tokens (Only show when not actively typing and we have data) */}
                          {!isActivelyTyping && addressString && (
                            <div className="flex items-center flex-wrap gap-2">
                              <span className="text-[11px] px-2 py-1 bg-white/5 border border-white/5 rounded-md text-white/70">{loc.street1}</span>
                              {loc.street2 && <span className="text-[11px] px-2 py-1 bg-white/5 border border-white/5 rounded-md text-white/70">{loc.street2}</span>}
                              <span className="text-[11px] px-2 py-1 bg-white/5 border border-white/5 rounded-md text-white/70">{loc.city}</span>
                              <span className="text-[11px] px-2 py-1 bg-white/5 border border-white/5 rounded-md text-white/70">{loc.state}</span>
                              <span className="text-[11px] px-2 py-1 bg-white/5 border border-white/5 rounded-md text-white/70">{loc.zip}</span>
                              <button onClick={() => update(loc.id, 'mode', 'manual')} className="text-[11px] text-white/40 hover:text-white/80 ml-1 flex items-center gap-1 transition-colors">
                        ...
                        <div className="flex flex-col gap-5">
                          <div className="flex items-center justify-between">
                            <span className="text-[10px] text-white/50 uppercase tracking-[0.08em] font-semibold">Manual Entry</span>
                            <button onClick={() => update(loc.id, 'mode', 'smart')} className="text-[11px] text-[#E91E8C] hover:text-[#E91E8C]/80 font-medium flex items-center gap-1.5 transition-colors">
                          ...
                          <div className="flex gap-4 flex-col sm:flex-row">
                            ...
                            <div className="sm:w-[140px]">
                              <label className="flex items-center justify-between text-[10px] text-white/50 uppercase tracking-[0.08em] font-semibold mb-1.5 h-[15px]">
                                ZIP 
      ...
      <button 
        onClick={add} 
        className="group flex items-center justify-center gap-2 text-[13px] font-medium text-white/40 hover:text-white border border-dashed border-white/10 hover:border-white/20 py-2.5 rounded-xl hover:bg-white/5 transition-all w-full mt-2"
      >
