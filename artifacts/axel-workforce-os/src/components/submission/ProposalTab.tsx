...
export default function ProposalTab({ dealId, dealName }: ProposalTabProps) {
  ...
  async function handleCreateProposal() {
    ...
    try {
      ...
      setRequestResult({ type: "error", message: err.message || "Failed to create proposal. Make sure a quote exists for this deal." });
    }
    setCreating(false);
  }
...
function PricingCard({ label, value, isAccent }: { label: string; value: string; isAccent?: boolean }) {
  return (
    <div style={{
      ...
      border: `1px solid ${isAccent ? "rgba(124,58,237,0.25)" : "rgba(255,255,255,0.07)"}`,
