...
interface Tracker {
  ...
  createdAt: string;
  completedAt?: string;
  deal?: { businessName?: string; stage?: string; id?: string };
}
...
export default function Implementations() {
  ...
  const [tab, setTab] = useState<"WC" | "PEO">("WC");
  ...
  const fetchTrackers = useCallback(async () => {
    ...
    const enriched = await Promise.all(
      rows.map(async (t) => {
        try {
          const deal = await api.get<{ businessName?: string; stage?: string; id?: string }>(`/deals/${t.dealId}`);
          return { ...t, deal };
        } catch {
  ...
  const filtered = trackers.filter((t) => {
    if (tab === "WC") return t.productType === "WC" || t.productType === "WC_BIND";
    return t.productType === "PEO" || t.productType === "PEO_ONBOARD";
  });

  const handleAdvancePhase = async (tracker: Tracker) => {
    const phases = tracker.productType === "PEO" || tracker.productType === "PEO_ONBOARD" ? PEO_PHASES : WC_PHASES;
    ...
    <div>
      ...
      <div style={{ display: "flex", gap: "4px", marginBottom: "24px" }}>
        {(["WC", "PEO"] as const).map((t) => (
          ...
          >
            {t === "WC" ? "WC Bind Journey" : "PEO / ASO Onboarding"}
          </button>
        ))}
      </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
        {filtered.length === 0 && (
          <GlassCard padding="40px" style={{ textAlign: "center" }}>
            ...
            <p style={{ color: textMuted, fontSize: "15px", margin: 0 }}>
              No {tab === "WC" ? "WC Bind Journey" : "PEO Onboarding"} trackers yet. Move a deal to "Bound" stage to create one.
            </p>
          </GlassCard>
        )}
        ...
        {filtered.map((tracker) => {
          ...
          const expectedDays = (tracker.productType === "PEO" || tracker.productType === "PEO_ONBOARD") ? 22 : 7;
