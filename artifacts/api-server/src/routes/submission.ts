...
router.post("/submit-for-approval", async (req, res) => {
  ...
  // Stage reflects how far the submission progressed (v2.7 §6 Segment 2):
  // an indication was generated by the time this endpoint fires with rating
  // data, so the deal lands in INDICATION; without rating data it stays in
  // SUBMISSION_REVIEW. U/W_REVIEW is set later, when a proposal is requested.
  ...
  // Lost deals and completed (Client) deals do not block a new submission.
  // The check and the insert share one transaction with a row lock on the
  // account, so two concurrent submissions for the same account serialize —
  // the second observes the first's deal and is blocked.
  const dealOutcome = await db.transaction(async (tx) => {
    await tx
    ...
    if (!accountCreated) {
      ...
      if (blocking) {
        await tx.insert(activityLogTable).values({
          ...
          description: `Duplicate submission attempt for ${businessName || account.businessName} blocked — active submission ${blocking.referenceCode} already in progress.`,
          metadata: { attempted_business_name: businessName ?? null, attempted_fein: fein ?? null },
        });
    ...
    const [inserted] = await tx
      ...
      .values({
        ...
        stage: hasIndication ? "INDICATION" : "SUBMISSION_REVIEW",
      ...
      .returning();
    return { kind: "created" as const, deal: inserted };
  });

  if (dealOutcome.kind === "blocked") {
    const { blocking } = dealOutcome;
    return res.status(409).json({
      error:
        "This company already has an active submission in progress " +
        `(${blocking.referenceCode}). A new submission cannot be started while one is active — ` +
        "please contact Axel to continue with the existing submission.",
      blockedByPriorSubmission: true,
      existingDealId: blocking.id,
      existingReferenceCode: blocking.referenceCode,
    });
  }
