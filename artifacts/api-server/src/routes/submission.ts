...
router.post("/submit-for-approval", async (req, res) => {
  ...
  // Stage placement (v2.7 §6 Segment 2 + stage table): "full journey
  // complete → U/W Review — full submission complete; awaiting underwriter."
  ...
  const dealOutcome = await db.transaction(async (tx) => {
    ...
    const [inserted] = await tx
      ...
      .values({
        ...
        stage: hasIndication ? "UW_REVIEW" : "SUBMISSION_REVIEW",
