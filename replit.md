...
- **Accent System (two-tier):** Pink `#E91E8C` is the **primary interactive accent** (buttons, links, focus rings, selected/active states, the active nav indicator bar, selected tab underlines, View All / ghost actions, loading). Purple `#7C3AED` is the **supporting accent** (icon chips, the AxelBadge `purple` variant, the secondary chart series, and category/role distinctions such as the non-ASO deal-card border and the ADMIN role label). The single primary CTA per screen is rendered via the `--gradient-cta` purple→pink gradient — the ONLY permitted gradient. No other gradients are allowed.
