...
- **Glassmorphism Modals:** Overlays use the shared tokens `--overlay-bg` (dark: `rgba(0,0,0,0.68)`, light: `rgba(15,15,20,0.40)`) + `--overlay-blur` (`blur(12px)`) — a frosted-glass backdrop that blurs and darkens the page behind every modal. shadcn Dialog/Sheet overlays consume them via the `.modal-overlay` utility class; inline-styled overlays use `var(--overlay-bg)`/`var(--overlay-blur)` directly (mirrored as `overlayBg`/`overlayBlur` in `use-theme-colors.ts`). Never use flat `rgba(0,0,0,0.5)` overlays. Popup panels keep `rgba(18,18,24,0.82)` background with `backdrop-filter: blur(40px)`, `box-shadow: 0 24px 80px rgba(0,0,0,0.6)`, and `inset 0 1px 0 rgba(255,255,255,0.06)` highlight.
...
- **Deal Card Modal:** Detailed full-screen modal for deal interaction, including activities, tasks, and editable details. Header features a black/grey minimalist US map background (`DealHeaderMap.tsx`, us-atlas + d3-geo) with glowing markers for submitted locations (employee-count chips; per-location FT+PT from the deal's quote `workforceProfile`, falling back to deal state × locations with an even headcount split via `src/lib/geo.ts` ZIP/state-centroid geocoding). The header band is ~248px tall (flex column; the 6-phase milestone tracker is pinned to the bottom via `marginTop: auto`), header KPIs LOCATIONS/EXMOD fall back to the quote `workforceProfile` (`locations.length` / `eMod`) when the deal-level columns are null, and map markers are clickable (keyboard-accessible `role="button"` SVG groups) — opening a glassmorphism popup anchored to the dot that lists that location's employee types (class-code description, FT/PT, annual payroll), dismissed via X, map-background click, or re-clicking the marker. Also includes a 38px team avatar row under the company name alternating stock headshots (`public/images/avatars/`) and grey initials circles, each wired to the UserMiniProfile hover card.
